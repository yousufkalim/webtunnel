...
const debug = require('debug')('webtunnel:client');
...
module.exports = class Tunnel extends EventEmitter {
      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.status !== 200) {
            const err = new Error(
              (body && body.message) ||
                'webtunnel server returned an error, please try again',
            );
            return cb(err);
          }
  ...
  _establish(info) {
    // increase max event listeners so that webtunnel consumers don't get
